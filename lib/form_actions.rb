module FormActions
  ...
  def self.add_responses(form, responses, student)
    ...
    correct = grade_and_create_responses(responses, student, form.category)
  ...
  def self.grade_and_create_responses(responses, student, category)
    correct = 0.0
    responses.each do |q_id, answer|
      correct += 1 if Question.find(q_id).answer.eql?(answer.to_i)
      student.responses.create!(question_id: q_id, answer: answer, category: category)
    end
    correct
  end
end
